...
== Secondary Indexes
http://redis.io/topics/indexes[Secondary indexes] are used to enable lookup operations based on native Redis structures. Values are written to the according indexes on every save and are removed when objects are deleted or xref:redis.repositories:redis.repositories.expirations.adoc#redis.repositories.expirations[expire].
...
CAUTION: Indexes cannot be resolved on xref:redis.repositories:redis.repositories.references.adoc#redis.repositories.references[References].
