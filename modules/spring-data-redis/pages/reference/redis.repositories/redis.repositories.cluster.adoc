...
You can use the Redis repository support in a clustered Redis environment. See the "`xref:cluster:cluster.adoc#cluster[cluster]`" section for `ConnectionFactory` configuration details. Still, some additional configuration must be done, because the default key distribution spreads entities and secondary indexes through out the whole cluster and its slots.
