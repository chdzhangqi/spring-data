...
The preceding example shows a simple domain object. You can use it to create an `Example`. By default, fields having `null` values are ignored, and strings are matched by using the store specific defaults. Examples can be built by either using the `of` factory method or by using xref:mongo.core:query-by-example.adoc#query-by-example.matchers[`ExampleMatcher`]. `Example` is immutable. The following listing shows a simple Example:
